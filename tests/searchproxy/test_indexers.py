...
def test_upsert_dataset(db, dataset, env, mocker):
    ...
    with db.scoped_session() as session:
        dataset_indexed = indexers.upsert_dataset(session, es={}, datasetUri=dataset.datasetUri)
...
def test_upsert_folder(db, dataset, env, mocker, folder):
    mocker.patch("dataall.searchproxy.upsert", return_value={})
    with db.scoped_session() as session:
        folder_indexed = indexers.upsert_folder(session, es={}, locationUri=folder.locationUri)
...
def test_upsert_tables(db, dataset, env, mocker, folder):
    mocker.patch("dataall.searchproxy.upsert", return_value={})
    with db.scoped_session() as session:
        tables = indexers.upsert_dataset_tables(session, es={}, datasetUri=dataset.datasetUri)
        assert len(tables) == 1
