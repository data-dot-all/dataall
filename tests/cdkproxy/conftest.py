...
def org(db) -> models.Organization:
    with db.scoped_session() as session:
        org = models.Organization(name="org", owner="me", label="org", description="test")
        session.add(org)
    yield org
...
@pytest.fixture(scope="module", autouse=True)
def pipeline(db, env: models.Environment) -> models.SqlPipeline:
    with db.scoped_session() as session:
...
def patch_ssm(mocker):
    mocker.patch("dataall.utils.parameter.Parameter.get_parameter", return_value="param")
