...
def patch_check_env(module_mocker):
    module_mocker.patch(
        'dataall.core.environment.services.environment_service.EnvironmentService.check_environment',
        return_value='CDKROLENAME',
    )
    module_mocker.patch(
        'dataall.base.aws.parameter_store.ParameterStoreManager.get_pivot_role_as_part_of_environment', return_value=False
    )
