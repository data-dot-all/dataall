...
def test_dashboard_count_votes(client, dashboard, env_fixture):
    response = count_votes_query(
        client, dashboard.dashboardUri, 'dashboard', env_fixture.SamlGroupName
    )
...
def test_dashboard_upvote(patch_es, client, env_fixture, dashboard):
    ...
    response = upvote_mutation(
        client, dashboard.dashboardUri, 'dashboard', True, env_fixture.SamlGroupName
    )
    ...
    response = upvote_mutation(
        client, dashboard.dashboardUri, 'dashboard', False, env_fixture.SamlGroupName
    )
