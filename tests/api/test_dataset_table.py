...
def test_update_table(client, env1, table, dataset1, db, user, group):
    table_to_update = table(dataset=dataset1, name=f"table_to_update", username=dataset1.owner)
    response = client.query(
        """
...
def test_update_dataset_table_column(client, table, dataset1, db):
    with db.scoped_session() as session:
        ...
        column = session.query(dataall.db.models.DatasetTableColumn).get(column.columnUri)
...
def test_sync_tables_and_columns(client, table, dataset1, db):
    with db.scoped_session() as session:
        ...
        assert dataall.db.api.DatasetTable.sync(session, dataset1.datasetUri, glue_tables)
...
def test_delete_table(client, table, dataset1, db, group):
    table_to_delete = table(dataset=dataset1, name=f"table_to_update", username=dataset1.owner)
    response = client.query(
        """
