...
def test_add_sm_user_profile(client, db, env1, org1, group, module_mocker):
    ...
    for i in range(0, 10):
        ...
        assert response.data.createSagemakerStudioUserProfile.SamlAdminGroupName == group.name
        assert response.data.createSagemakerStudioUserProfile.environmentUri == env1.environmentUri
...
def test_nopermissions_list_sagemaker_studio_user_profiles(client, env1, db, org1, group):
    response = client.query(
        """
        query listSagemakerStudioUserProfiles($filter:SagemakerStudioUserProfileFilter!){
            listSagemakerStudioUserProfiles(filter:$filter){
...
def test_delete_sagemaker_studio_user_profile(client, env1, db, org1, module_mocker, group):
    with db.scoped_session() as session:
        sm_user_profile = session.query(dataall.db.models.SagemakerStudioUserProfile).first()
