...
def test_dataset_in_environment(client, env1, dataset1, group):
    ...
    """
    response = client.query(q, username=env1.owner, groups=[group.name], environmentUri=env1.environmentUri)
...
def test_delete_dataset(client, dataset, env1, org1, db, module_mocker, group, user):
    with db.scoped_session() as session:
        ...
        session.commit()
    deleted_dataset = dataset(org=org1, env=env1, name="dataset1", owner=user.userName, group=group.name)
