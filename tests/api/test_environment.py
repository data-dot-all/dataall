...
def test_get_environment(client, org1, env1, group):
    ...
    assert response.data.getEnvironment.organization.organizationUri == org1.organizationUri
...
def test_update_env(client, org1, env1, group):
    ...
    print(response)
    assert response.data.updateEnvironment.organization.organizationUri == org1.organizationUri
...
def test_group_invitation(db, client, env1, org1, group2, user, group3, group, dataset):
    ...
    dataset = dataset(org=org1, env=env1, name="dataset1", owner="bob", group=group2.name)
...
def test_create_environment(db, client, org1, env1, user, group):
    ...
    assert response.data.createEnvironment.EnvironmentDefaultIAMRoleName == "myOwnIamRole"
    ...
    with db.scoped_session() as session:
        env = dataall.db.api.Environment.get_environment_by_uri(session, response.data.createEnvironment.environmentUri)
        session.delete(env)
        session.commit()
