...
def patch_aws(module_mocker):
    ...
    module_mocker.patch("dataall.aws.handlers.sagemaker.Sagemaker.stop_instance", return_value=True)
    module_mocker.patch(
...
def test_nopermissions_list_notebooks(client, env1, db, org1, user2, group2, sgm_notebook, patch_aws):
    response = client.query(
        """
        query ListSagemakerNotebooks($filter:SagemakerNotebookFilter){
            listSagemakerNotebooks(filter:$filter){
