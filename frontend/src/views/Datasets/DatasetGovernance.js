...
import { Box, Card, CardContent, CardHeader, Chip, Divider, Typography } from '@material-ui/core';
import Label from '../../components/Label';

const DatasetGovernance = (props) => {
  const { dataset } = props;
  const terms = dataset.terms.nodes.length > 0 ? dataset.terms.nodes : [{ label: '-', nodeUri: '-' }];
  ...
  return (
    <Card {...dataset}>
      ...
      <CardContent>
        ...
        <Box sx={{ mt: 1 }}>
          {dataset.topics && dataset.topics.length > 0 && dataset.topics.map((t) => (
            <Chip
              sx={{ mr: 0.5, mb: 0.5 }}
              key={t}
              label={t}
