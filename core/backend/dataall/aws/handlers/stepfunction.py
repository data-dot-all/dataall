...
def run_pipeline(state_machine_name, env: models.Environment, stage='Test'):
    if not state_machine_name:
        raise Exception(
            'An error occurred (StackNotFound) when calling the RUN PIPELINE operation'
        )
    ...
    try:
        ...
        raise Exception(
            f'An error occurred (StateMachineNotFound) {arn} when calling the RUN PIPELINE operation'
        )
