...
| custom_auth                                   | Optional              | A dictionary containing set of parameters to setup External Idp Authentication in data.all. Custom Auth Configuration : `provider`, `url`, `redirect_url`, `client_id`, `response_types`, `scopes`, `jwks_url`, `claims_mapping` (Nested dictionary containing configuration : `user_id`, `email`). All of the configuration are required if setting data.all with an external OIDC supported Idp|
    ...
    "DeploymentEnvironments": [
        {
            ...
            "prod_sizing": false,
             "custom_auth": {
               "provider": "okta",
               "url": "https://URL",
               "redirect_url": "https://redirect_url.net",
...
Please read this section only if you are deploying data.all with an external Idp.

Out of the box, data.all uses Cognito as the Idp and the user pool provider. Data.all can also be configured to use an external Idp
...
custom authorizer lambda and attach it to the API gateway. This custom authorizer will lambda will work in place of the cognito authorizer,
which is deployed by default. 

**Important** - 
When setting up data.all, the user pool is managed by Cognito. Here you can make teams and groups users in a team. This forms the data.all team.
...
class MyProvider(ServiceProvider):
    def get_user_emailids_from_group(self, role_name):
        # Your implementation
...
Once you implement these methods, create an instance of your provider in dataall/base/service/ServiceProviderFactory.py. Please follow the 
instructions in the comments to create and return and instance of your custom service provider.
