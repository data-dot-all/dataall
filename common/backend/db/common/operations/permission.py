...
class Permission:
    ...
    def paginated_tenant_permissions(session, data) -> dict:
        if not data:
            data = dict()
        data['type'] = models.PermissionType.TENANT
    ...
    def paginated_resource_permissions(session, data) -> dict:
        if not data:
            data = dict()
        data['type'] = models.PermissionType.RESOURCE
        return Permission.paginated_permissions(session, data)
    ...
    def init_permissions(session):
        ...
        count_resource_permissions = (
            session.query(models.Permission)
            .filter(models.Permission.type == models.PermissionType.RESOURCE.name)
            .count()
        )
            ...
            for name, desc in permissions.RESOURCES_ALL_WITH_DESC.items():
                perms.append(
                    Permission.save_permission(
                        ...
                        name=name,
                        description=desc,
                        permission_type=models.PermissionType.RESOURCE.name,
                    )
                )
        ...
        count_tenant_permissions = (
            session.query(models.Permission)
            .filter(models.Permission.type == models.PermissionType.TENANT.name)
            .count()
        )
