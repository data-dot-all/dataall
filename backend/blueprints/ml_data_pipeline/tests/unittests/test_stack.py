...
def test_stack():
    ...
    assert isinstance(stack.layer_versions.get("datascience_layer"), aws_cdk.aws_lambda.ILayerVersion)
    assert isinstance(stack.layer_versions.get("numpy_scipy37"), aws_cdk.aws_lambda.ILayerVersion)
    ...
    assert stack.state_machine_definition.end_states[0].id == f"Lambda: Tag Model model-from-{job_name}"


def test_stack_aws_resources_only():
    """Test the case where the definition of configuration file contains only aws resource"""
    app = core.App()
    pipeline = TaskGroupReader(path="tests/unittests/config_files/config_resource_only.yaml")
    stack = DataPipeline(app, "pipeline", pipeline, description="testpipeline")
...
def test_stack_no_resources():
    ...
    app = core.App()
    pipeline = TaskGroupReader(path="tests/unittests/config_files/config_no_resource.yaml")
    stack = DataPipeline(app, "pipeline", pipeline, description="testpipeline")
...
def test_error_when_layer_not_defined():
    ...
    try:
        app = core.App()
        pipeline = TaskGroupReader(path="tests/unittests/config_files/config_resource_err.yaml")
...
def test_training_complete():
    ...
    app = core.App()
    pipeline = TaskGroupReader(path="tests/unittests/config_files/config_training_complete.yaml")
    stack = DataPipeline(app, "pipeline", pipeline, description="testpipeline")
    assert stack.state_machine
...
def test_api_gateway():
    app = core.App()
    pipeline = TaskGroupReader(path="tests/unittests/config_files/config_with_api_gateway.yaml")
    stack = DataPipeline(app, "pipeline", pipeline, description="testpipeline")
...
def test_schedule_step_function():
    ...
    app = core.App()
    pipeline = TaskGroupReader(path="tests/unittests/config_files/config_with_schedules.yaml")
    stack = DataPipeline(app, "pipeline", pipeline, description="testpipeline")
