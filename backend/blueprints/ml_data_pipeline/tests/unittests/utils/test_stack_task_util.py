...
class PipelineStack(core.Stack):
    def __init__(self, scope, pipeline, id, **kwargs):
        ...
        self.pipeline_iam_role_arn = "arn:aws:iam::012345678912:role/dhdatasciencedevoqtnpj"
...
def test_make_step_function_task():
    pipeline = TaskGroupReader(path="tests/unittests/config_files/config_all_tasks.yaml")
    ...
    assert stack.tasks[3].start_state.id == "Lambda: Delete Model " + model_job_name + " If exists"
    ...
    assert stack.tasks[4].start_state.id == "Lambda: Delete Endpoint Config " + end_point_cfg_job_name + " If exists"
    assert stack.tasks[4].end_states[0].id == "Lambda: Tag Endpoint Config " + end_point_cfg_job_name
...
def test_make_step_function_task_training():
    ...
    assert isinstance(stack.tasks[0], aws_cdk.aws_stepfunctions_tasks.LambdaInvoke)
    assert stack.tasks[0].id == f'Lambda: {pipeline.definition["groups"][0]["jobs"][0].get("name")}'
    assert isinstance(stack.tasks[1], aws_cdk.aws_stepfunctions.CustomState)
    assert stack.tasks[1].id == f'Task {pipeline.definition["groups"][1]["jobs"][0].get("name")}'
    assert isinstance(stack.tasks[2], aws_cdk.aws_stepfunctions.Chain)
    assert stack.tasks[2].id == f'Lambda: Tag Model {pipeline.definition["groups"][2]["jobs"][0].get("name")}'
