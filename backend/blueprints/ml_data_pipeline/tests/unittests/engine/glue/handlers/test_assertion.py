...
def test_evaluate_query(spark_session):
    ...
    assertion_handler = assertion.Assertion(
        type="assertion", name="my_assertion", config={"sql": "SELECT * FROM {{dataframe}} WHERE age <= 0"}
    )
    ...
    context = Context()
    assertion_handler_1 = assertion.Assertion(
        type="assertion", name="my_assertion", config={"sql": "SELECT * FROM {{dataframe}} WHERE age <= 0"}
    )
    assertion_handler_1.run_step(spark_session, config_reader, context)

    assertion_handler_2 = assertion.Assertion(
        type="assertion",
        name="my_assertion",
        config={"sql": "SELECT * FROM {{dataframe}} WHERE age <= 0", "sign": "eq"},
    )
