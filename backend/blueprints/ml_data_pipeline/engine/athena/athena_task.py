...
def read_query(query, workgroup):
    if query.startswith("States.Format"):  # For dynamic sql strings and prepared statements
        definition = {
            "Type": "Task",
...
def make_athena_query_task(stack, job):
        ...
        retry_definition = [
            {
                ...
                "IntervalSeconds": job.get("config").get("retry").get("interval_seconds", 1),
    ...
    for step, query_list in zip(config.steps, config.queries):
        ...
        for job, query in zip(step.get("jobs"), query_list):
            ...
            task = stepfunctions.CustomState(stack, "Athena: " + job["name"], state_json=definition)
