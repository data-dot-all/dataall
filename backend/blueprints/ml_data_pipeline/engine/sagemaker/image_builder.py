...
class SageMakerImageBuilder:
        ...
        try:
            if main_modules:
                ...
                for main_module in main_modules:
                    ...
                    tag_name = SageMakerImageBuilder.build_training_job_image(main_module, image_uri, token)
    ...
    def build_image_tag(image_uri, module_path, tag_name):
        ...
        dockerfile_path = os.path.realpath(
            os.path.abspath(os.path.join(__file__, "..", "..", "..", "smjobs", "Dockerfile"))
        )
        subprocess.run(
            [
                f"docker",
...
if __name__ == "__main__":
    SageMakerImageBuilder.build_training_jobs_images(os.getenv("ECR_REPOSITORY_URI"), os.getenv("AWSREGION"))
