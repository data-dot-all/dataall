...
class PrebuiltExtensionImageBuilder:
        ...
        try:
            if docker_infos:
                ...
                for docker_info in docker_infos:
                    ...
                    base_image_uri = "{}/{}:{}".format(base_container_server, base_image, base_container_tag)

                    main_module = docker_info["module"]
                    entry_point = docker_info["entry_point"]
                    dockerfile = docker_info.get("dockerfile")

                    print("To login to {} to obtain the base container".format(base_container_server))
    ...
    def image_uri_from_main_module(main_module):
        return os.getenv("ECR_REPOSITORY") + ":" + PrebuiltExtensionImageBuilder.get_image_tag(main_module)

    @staticmethod
    def build_training_job_image(main_module, base_image, image_uri, entry_point, dockerfile, token=None):
        try:
            ...
            if token:
                print("To push image  {} to ECR with tag {} ".format(image_uri, tag_name))
                PrebuiltExtensionImageBuilder.push_image_to_ecr(image_uri, tag_name, token)
    ...
    def push_image_to_ecr(image_uri, tag_name, token):
        ...
        subprocess.run([f"docker", "push", f"{image_uri}:{tag_name}"], stdout=subprocess.PIPE)
    ...
    def build_image_tag(
        ...
        dockerfile_path = os.path.realpath(
            os.path.abspath(os.path.join(__file__, "..", "..", "..", "sagemaker_jobs", "dockerfiles", dfile))
        )
        subprocess.run(
            [
                f"docker",
    ...
    def get_prebuilt_docker_infos_of_group(group, docker_infos):
        for j in group.get("jobs", []):
            if j.get("type") == "sagemaker_training" and j["config"].get("algorithm", {}).get("pre_built"):
                ...
                for choice in j.get("choices", []):
                    for sub_group in choice.get("groups", []):
                        PrebuiltExtensionImageBuilder.get_prebuilt_docker_infos_of_group(sub_group, docker_infos)
    ...
    def get_prebuilt_docker_infos():
        ...
        for group in groups.definition.get("groups", []):
            PrebuiltExtensionImageBuilder.get_prebuilt_docker_infos_of_group(group, docker_infos)
        return docker_infos

    @staticmethod
    def get_groups():
        configfile_path = os.path.realpath(os.path.abspath(os.path.join(__file__, "..", "..", "..", "config.yaml")))
        groups = TaskGroupReader(path=configfile_path)
        return groups


if __name__ == "__main__":
    PrebuiltExtensionImageBuilder.build_training_jobs_images(os.getenv("ECR_REPOSITORY"), os.getenv("AWSREGION"))
