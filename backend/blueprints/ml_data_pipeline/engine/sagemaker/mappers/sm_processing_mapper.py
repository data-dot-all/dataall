...
class SageMakerProcessingJobPropsMappingException(Exception):
    def __init__(self, error, arg_name):
        super().__init__()
        self.message = f"Failed to create SageMakerProcessingJobProps due to error: `{error}`"
...
class SageMakerProcessingJobPropsMapper:
    ...
    def map_props(cls, stack, job_name, main_module, config_props, tags) -> dict:
        ...
        app_specification = SageMakerProcessingJobPropsMapper.map_app_specification(config_props, main_module, stack)

        environment = f"{config_props.get('environment')}" if config_props.get("environment") else None
        ...
        if not props["ProcessingJobName.$"]:
            raise SageMakerProcessingJobPropsMappingException(Exception, "Job Name is Mandatory")
        if not props["AppSpecification"]:
            raise SageMakerProcessingJobPropsMappingException(Exception, "AppSpecification is Mandatory")
        if not props["ProcessingResources"]:
            raise SageMakerProcessingJobPropsMappingException(Exception, "ProcessingResources is Mandatory")
        if not props["RoleArn"]:
            raise SageMakerProcessingJobPropsMappingException(Exception, "RoleArn is Mandatory")
    ...
    def map_stopping_condition(cls, config_props):
        if config_props.get("stopping_condition"):
            stopping_condition = {"MaxRuntimeInSeconds": config_props["stopping_condition"]["max_runtime"]}
            return stopping_condition

    @classmethod
    def map_processing_resources(cls, config_props):
        processing_resources = {"ClusterConfig": dict(InstanceCount=1, InstanceType="ml.m4.xlarge", VolumeSizeInGB=35)}
        if config_props.get("processing_resources"):
            processing_resources = {
                "ClusterConfig": {
                    "InstanceCount": config_props["processing_resources"]["cluster_config"]["instance_count"],
                    "InstanceType": config_props["processing_resources"]["cluster_config"]["instance_type"],
                    "VolumeSizeInGB": config_props["processing_resources"]["cluster_config"]["volume_size"],
                    "VolumeKmsKeyId": config_props["processing_resources"]["cluster_config"].get(
    ...
    def map_processing_output_config(cls, config_props):
        if config_props.get("processing_output_config"):
            ...
            processing_output_config["KmsKeyId"] = config_props["processing_output_config"]["kms_key_id"]
            outputs = []
            ...
            processing_output_config = cls.clean_dict_from_none_values(processing_output_config)
                ...
                processing_inputs.append(
                    {
                        ...
                        "S3Input": {
                            ...
                            "S3DataDistributionType": p["s3_input"]["s3_data"]["distribution_type"],
    ...
    def map_network_config(cls, config_props):
        if config_props.get("network_config"):
            network_config = {
                "EnableInterContainerTrafficEncryption": config_props.get("network_config").get(
                    "enable_intercontainer_traffic_encryption"
                ),
                "EnableNetworkIsolation": config_props.get("network_config").get("enable_network_isolation"),
                "VpcConfig": {
                    "SecurityGroupIds": config_props.get("network_config").get("vpc_config")["security_groups"],
                    "Subnets": config_props.get("network_config").get("vpc_config")["subnets"],
                },
            }
    ...
    def map_app_specification(cls, config_props, main_module, stack):
        ...
        if config_props.get("app_specification"):
            ...
            if config_props["app_specification"].get("container_entrypoint"):
                container_entrypoint = config_props["app_specification"]["container_entrypoint"]
            if config_props["app_specification"].get("container_arguments"):
                container_arguments = config_props["app_specification"]["container_arguments"]
