...
def make_step_function_task(stack, job, group_index=-1, job_index=-1):
    ...
    if job.get("type") == "glue_job":
        task = make_glue_job_task(stack, job, stage=stack.stage, bucket_name=stack.bucket_name)
        ...
        for job_index, job in enumerate(group.get("jobs")):
            ...
            if isinstance(task, list) and job.get("type") != "choice":
                ...
                for index, step_task_list in enumerate(task):
                    ...
                    append_step_function_parallels(stack, step_parallels, step_task_list, group, name)
                    index = index + 1
...
def compile_step_function_definition(parallels, job_name_generator_task, use_name_generator_task):
    ...
    """
    definition = job_name_generator_task if (job_name_generator_task and use_name_generator_task) else parallels[0]
...
def build_step_function_definition(stack, pipeline_definition, job_name_generator_task, use_name_generator_task):
    ...
    definition = compile_step_function_definition(parallels, job_name_generator_task, use_name_generator_task)
...
def make_choice_task(stack, job):
    ...
    for i, choice_config in enumerate(job["choices"]):
        ...
        target_task, first_state, next_state = build_step_function_definition(stack, sub_job, None, False)
        if condition_type == "BooleanEquals":
            ...
            raise Exception("Unknown condition {} at choice task".format(condition_type))
            ...
            payload=stepfunctions.TaskInput.from_object(
                {
                    "jobs": job_names,
                    "tags": tags,
                    "execution_id": stepfunctions.TaskInput.from_data_at("$$.Execution.Name").value,
                }
            ),
