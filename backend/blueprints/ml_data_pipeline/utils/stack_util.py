...
def get_parameter_value(param_name):
    ...
    """
    param_value = boto3.client("ssm").get_parameter(Name=param_name)["Parameter"]["Value"]
    return param_value
...
def build_object_attributes(stack, dictionary, pipeline):
    ...
    stack.pipeline_name_origin = dictionary.get("ORIGIN_PIPELINE_NAME", pipeline.definition.get("name"))
    stack.pipeline_name = dictionary.get("PIPELINE_NAME", pipeline.definition.get("name"))
    ...
    stack.bucket_name = attr_from_env_paramstore(stack, "BUCKET_NAME", "bucket_name", dictionary)

    # The account ID where the pipeline is defined.
    stack.accountid = attr_from_env_paramstore(stack, "AWSACCOUNTID", "accountid", dictionary)

    # The repository URI where the docker images corresponding to this pipeline is stored.
    stack.ecr_repository_uri = attr_from_env_paramstore(stack, "ECR_REPOSITORY", "ecr_repository_uri", dictionary)

    # The default role of the pipeline.
    stack.pipeline_iam_role_arn = attr_from_env_paramstore(stack, "ENVROLEARN", "pipeline_iam_role_arn", dictionary)
    ...
    stack.pipeline_fulldev_iam_role = attr_from_env_paramstore(stack, "FULLDEVROLEARN", "fulldev_iam_role", dictionary)

    stack.pipeline_admin_iam_role = attr_from_env_paramstore(stack, "ADMINROLEARN", "admin_iam_role", dictionary)

    # The region in which the pipeline runs
    stack.pipeline_region = attr_from_env_paramstore(stack, "AWS_DEFAULT_REGION", "pipeline_region", dictionary)
...
def create_step_function_triggers(stack, pipeline_definition, state_machine_arn):
    ...
    for r in pipeline_definition.get("triggers"):
        ...
        if r.get("type") not in {"lambda_function"}:  # we only support lambdafx as trigger at the moment
...
## Scheduler
def create_event_rule_for_scheduler(stack, scheduler_config, lambda_fn, state_fn=None, rule_name="rule"):
    ...
    if scheduler_config.get("cron"):
        ...
        raise ResourceCreationException("Unknown scheduler {}".format(str(scheduler_config)), "lambdafx")
...
def create_step_function_scheduler(stack, pipeline_definition, sfn):
    ...
    if pipeline_definition.get("properties", {}).get("scheduler"):
        ...
        rules = [create_event_rule_for_scheduler(stack, scheduler_config, None, sfn, rule_name)]
    else:
        rules = [
            create_event_rule_for_scheduler(stack, scheduler_config, None, sfn, f"{rule_name}_{i}")
            for i, scheduler_config in enumerate(pipeline_definition.get("properties", {}).get("schedulers", []))
        ]
...
def create_aws_resources(stack, pipeline):
    ...
    for resource in pipeline.definition.get("aws_resources", []):
        if resource["type"] == "dynamodb":
            ...
            stack.resources[resource.get("name")] = make_lambda_layer_version(stack, resource)
        elif resource["type"] == "lambda_function":
            stack.resources[resource.get("name")] = make_lambda_python_function(stack, resource)
        elif resource["type"] == "athena_workgroup":
            stack.resources[resource.get("name")] = make_athena_workgroup(stack, resource)
        elif resource["type"] == "athena_prepared_statement":
            stack.resources[resource.get("name")] = make_athena_prepared_statement(stack, resource)
            ...
            stack.resources[resource.get("name")] = make_batch_job_definition(stack, resource)
        elif resource["type"] == "batch_job_queue":
            stack.resources[resource.get("name")] = make_batch_job_queue(stack, resource)
        elif resource["type"] == "batch_compute_environment":
            stack.resources[resource.get("name")] = make_batch_compute_environment(stack, resource)
        elif resource["type"] == "sagemaker_model_package_group":
            stack.resources[resource.get("name")] = make_sagemaker_model_package_group(stack, resource)
        else:
...
def create_resource_tags(stack, dictionary):
    ...
    """
    tags = dict([(k[len("ENVTAG_") :], v) for k, v in dictionary.items() if k.startswith("ENVTAG_")])
