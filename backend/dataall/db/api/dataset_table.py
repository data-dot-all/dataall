...
class DatasetTable:
    ...
    def create_dataset_table(
        ...
        if data.get("terms") is not None:
            Glossary.set_glossary_terms_links(session, username, table.tableUri, "DatasetTable", data.get("terms", []))
        session.commit()
        return table
    ...
    def delete_dataset_table(
        ...
        Glossary.delete_glossary_terms_links(session, target_uri=table.tableUri, target_type="DatasetTable")
        return True

    @staticmethod
    def get_dataset_tables_shared_with_env(session, environment_uri: str, dataset_uri: str, status: List[str]):
        ...
        env_tables = (
            ...
            .filter(
                and_(
                    ...
                    models.ShareObject.environmentUri == environment_uri,  # for this environment
    ...
    def sync(session, datasetUri, glue_tables=None):
        ...
        if dataset:
            existing_tables = (
                session.query(models.DatasetTable).filter(models.DatasetTable.datasetUri == datasetUri).all()
            )
            ...
            for table in glue_tables:
                if table["Name"] not in existing_table_names:
                    logger.info(f"Storing new table: {table} for dataset db {dataset.GlueDatabaseName}")
                    ...
                    logger.info(f"Updating table: {table} for dataset db {dataset.GlueDatabaseName}")
                    updated_table: models.DatasetTable = existing_dataset_tables_map.get(table["Name"])
    ...
    def update_existing_tables_status(existing_tables, glue_tables):
        for existing_table in existing_tables:
            if existing_table.GlueTableName not in [t["Name"] for t in glue_tables]:
                ...
                logger.info(f"Table {existing_table.GlueTableName} status set to Deleted from Glue.")
    ...
    def delete_all_table_columns(session, dataset_table):
        session.query(models.DatasetTableColumn).filter(
            and_(
                models.DatasetTableColumn.GlueDatabaseName == dataset_table.GlueDatabaseName,
