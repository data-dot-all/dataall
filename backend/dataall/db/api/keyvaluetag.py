...
class KeyValueTag:
    ...
    @staticmethod
    def list_key_value_tags(session, username, groups, uri, data=None, check_perm=None) -> dict:
        ResourcePolicy.check_user_resource_permission(
            session=session,
            username=username,
            groups=groups,
            resource_uri=uri,
            permission_name=TargetType.get_resource_read_permission_name(data["targetType"]),
        )
