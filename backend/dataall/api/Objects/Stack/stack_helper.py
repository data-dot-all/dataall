...
def get_stack_with_cfn_resources(context: Context, targetUri: str, environmentUri: str):
    with context.engine.scoped_session() as session:
        ...
        stack: models.Stack = db.api.Stack.find_stack_by_target_uri(session, target_uri=targetUri)
        if not stack:
            stack = models.Stack(
                stack="environment",
                payload={},
...
def deploy_stack(context, targetUri):
    with context.engine.scoped_session() as session:
        stack: models.Stack = db.api.Stack.get_stack_by_target_uri(session, target_uri=targetUri)
        ...
        if envname in ["local", "pytest", "dkrcompose"]:
            ...
            if not Ecs.is_task_running(cluster_name, f"awsworker-{stack.stackUri}"):
                ...
                task: models.Task = models.Task(action="ecs.cdkproxy.deploy", targetUri=stack.stackUri)
                session.add(task)
                session.commit()
...
def delete_stack(context, target_uri, accountid, cdk_role_arn, region, target_type=None):
    with context.engine.scoped_session() as session:
        stack: models.Stack = db.api.Stack.find_stack_by_target_uri(session, target_uri=target_uri)
        if not stack:
            return
        task = models.Task(
            targetUri=target_uri,
