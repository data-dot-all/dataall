...
def list_table_columns(
    ...
    return paginate(q, page=filter.get("page", 1), page_size=filter.get("pageSize", 65)).to_dict()


def sync_table_columns(context: Context, source, tableUri: str = None):
    with context.engine.scoped_session() as session:
        table: models.DatasetTable = db.api.DatasetTable.get_dataset_table_by_uri(session, tableUri)
...
def resolve_terms(context, source: models.DatasetTableColumn, **kwargs):
    if not source:
        return None
    with context.engine.scoped_session() as session:
        q = session.query(models.TermLink).filter(models.TermLink.targetUri == source.columnUri)
...
def update_table_column(context: Context, source, columnUri: str = None, input: dict = None):
    with context.engine.scoped_session() as session:
        column: models.DatasetTableColumn = session.query(models.DatasetTableColumn).get(columnUri)
        if not column:
            raise db.exceptions.ObjectNotFound("Column", columnUri)
        table: models.DatasetTable = db.api.DatasetTable.get_dataset_table_by_uri(session, column.tableUri)
        ResourcePolicy.check_user_resource_permission(
            session=session,
            username=context.username,
            groups=context.groups,
        ...
        task = models.Task(action="glue.table.update_column", targetUri=column.columnUri)
        session.add(task)
        session.commit()
