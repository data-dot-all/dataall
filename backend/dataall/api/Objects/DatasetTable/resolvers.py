...
def preview(context, source, tableUri: str = None):
    with context.engine.scoped_session() as session:
        ...
        dataset = db.api.Dataset.get_dataset_by_uri(session, table.datasetUri)
        if dataset.confidentiality != models.ConfidentialityClassification.Unclassified.value:
        ...
        try:
            env_workgroup = boto3_session.client("athena", region_name=env.region).get_work_group(
...
def get_glue_table_properties(context: Context, source: models.DatasetTable, **kwargs):
    if not source:
        return None
    with context.engine.scoped_session() as session:
        table: models.DatasetTable = db.api.DatasetTable.get_dataset_table_by_uri(session, source.tableUri)
...
def resolve_glossary_terms(context: Context, source: models.DatasetTable, **kwargs):
    if not source:
        return None
    with context.engine.scoped_session() as session:
        return Glossary.get_glossary_terms_links(session, source.tableUri, "DatasetTable")
...
def resolve_redshift_copy_location(context, source: models.DatasetTable, clusterUri: str):
    with context.engine.scoped_session() as session:
        return db.api.RedshiftCluster.get_cluster_dataset_table(
            session, clusterUri, source.datasetUri, source.tableUri
        ).dataLocation
