...
def ls(engine: Engine, task: models.Task):
    with engine.scoped_session() as session:
        ...
        for sub_folder in response["subFolders"]:
            ...
            get_commit = client.get_commit(repositoryName=pipe.repo, commitId=get_folder_response["commitId"])
        ...
        for file in response["files"]:
            ...
            get_commit = client.get_commit(repositoryName=pipe.repo, commitId=get_file_response["commitId"])
            commit = get_commit["commit"]
            nodes.append(
                {
                    "type": "file",
