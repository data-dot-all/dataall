...
class Sagemaker:
    ...
    def get_notebook_instance_status(AwsAccountId, region, NotebookInstanceName):
        try:
            ...
            response = client.describe_notebook_instance(NotebookInstanceName=NotebookInstanceName)
            return response.get("NotebookInstanceStatus", "NOT FOUND")
        except ClientError as e:
            logger.error(f"Could not retrieve instance {NotebookInstanceName} status due to: {e} ")
            return "NOT FOUND"
    ...
    def presigned_url(AwsAccountId, region, NotebookInstanceName):
        try:
            client = Sagemaker.client(AwsAccountId, region)
            response = client.create_presigned_notebook_instance_url(NotebookInstanceName=NotebookInstanceName)
    ...
    def start_instance(AwsAccountId, region, NotebookInstanceName):
        try:
            client = Sagemaker.client(AwsAccountId, region)
            status = Sagemaker.get_notebook_instance_status(AwsAccountId, region, NotebookInstanceName)
    ...
    def get_security_groups(AwsAccountId, region):
        try:
            ...
            sgindex = [i for i, s in enumerate(sgnames) if "DefaultLinuxSecurityGroup" in s]
