...
def get_pipeline_execution(engine: Engine, task: models.Task):
    with engine.scoped_session() as session:
        ...
        sqlpipeline: models.SqlPipeline = session.query(models.SqlPipeline).get(task.targetUri)
        ...
        executions = []
        try:
            response = codepipeline_client.list_pipeline_executions(pipelineName=codepipeline_name)
