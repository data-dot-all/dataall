...
def get_target_glossary_terms(session, targetUri):
    q = (
        ...
        .join(models.GlossaryNode, models.GlossaryNode.nodeUri == models.TermLink.nodeUri)
        .filter(
            and_(
...
def upsert_dataset(session, es, datasetUri: str):
    ...
    count_upvotes = db.api.Vote.count_upvotes(session, None, None, datasetUri, {"targetType": "dataset"})
...
def upsert_dashboard(session, es, dashboardUri: str):
    ...
    if dashboard:
        ...
        count_upvotes = db.api.Vote.count_upvotes(session, None, None, dashboardUri, {"targetType": "dashboard"})
        upsert(
            es=es,
            index="dataall-index",
            id=dashboardUri,
...
def upsert_dataset_tables(session, es, datasetUri: str):
    tables = session.query(models.DatasetTable).filter(models.DatasetTable.datasetUri == datasetUri).all()
    for table in tables:
