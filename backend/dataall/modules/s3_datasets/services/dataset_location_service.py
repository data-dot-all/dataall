...
from dataall.modules.datasets_base.services.dataset_base_permissions import MANAGE_DATASETS
...
class DatasetLocationService:
    ...
    def remove_storage_location(uri: str = None):
        with get_context().db_engine.scoped_session() as session:
            location = DatasetLocationRepository.get_location_by_uri(session, uri)
            has_shares = ShareObjectBaseRepository.has_shared_items(session, location.locationUri)
            if has_shares:
            ...
            ShareObjectBaseRepository.delete_shares(session, location.locationUri)
