...
def create_dataset(context: Context, source, input=None):
    ...
    return S3DatasetService.create_dataset(uri=uri, admin_group=admin_group, data=input)
...
def import_dataset(context: Context, source, input=None):
    ...
    admin_group = input['SamlAdminGroupName']
    uri = input['environmentUri']
    return S3DatasetService.import_dataset(uri=uri, admin_group=admin_group, data=input)


def get_dataset(context, source, datasetUri=None):
    return S3DatasetService.get_dataset(uri=datasetUri)
...
def get_file_upload_presigned_url(context, source, datasetUri: str = None, input: dict = None):
    return S3DatasetService.get_file_upload_presigned_url(uri=datasetUri, data=input)
...
def list_locations(context, source: S3Dataset, filter: dict = None):
    ...
    return S3DatasetService.list_locations(source.datasetUri, filter)
...
def list_tables(context, source: S3Dataset, filter: dict = None):
    ...
    if not filter:
        filter = {'page': 1, 'pageSize': 5}
    return S3DatasetService.list_tables(source.datasetUri, filter)
...
def update_dataset(context, source, datasetUri: str = None, input: dict = None):
    return S3DatasetService.update_dataset(uri=datasetUri, data=input)


def get_dataset_statistics(context: Context, source: S3Dataset, **kwargs):
    if not source:
        return None
    return S3DatasetService.get_dataset_statistics(source)
...
def get_dataset_assume_role_url(context: Context, source, datasetUri: str = None):
    return S3DatasetService.get_dataset_assume_role_url(uri=datasetUri)
...
def start_crawler(context: Context, source, datasetUri: str, input: dict = None):
    return S3DatasetService.start_crawler(uri=datasetUri, data=input)
...
def list_dataset_share_objects(context, source, filter: dict = None):
    ...
    if not filter:
        filter = {'page': 1, 'pageSize': 5}
    return S3DatasetService.list_dataset_share_objects(source, filter)


@is_feature_enabled('modules.s3_datasets.features.aws_actions')
def generate_dataset_access_token(context, source, datasetUri: str = None):
    return S3DatasetService.generate_dataset_access_token(uri=datasetUri)


def get_dataset_stack(context: Context, source: S3Dataset, **kwargs):
    if not source:
        return None
...
def delete_dataset(context: Context, source, datasetUri: str = None, deleteFromAWS: bool = False):
    return S3DatasetService.delete_dataset(uri=datasetUri, delete_from_aws=deleteFromAWS)
...
def list_datasets_created_in_environment(context: Context, source, environmentUri: str = None, filter: dict = None):
    if not filter:
        filter = {}
    return S3DatasetService.list_datasets_created_in_environment(uri=environmentUri, data=filter)
...
def list_datasets_owned_by_env_group(
    ...
    if not filter:
        filter = {}
    return S3DatasetService.list_datasets_owned_by_env_group(environmentUri, groupUri, filter)
...
def verify_dataset_share_objects(context: Context, source, input):
    ...
    return S3DatasetService.verify_dataset_share_objects(uri=dataset_uri, share_uris=verify_share_uris)
