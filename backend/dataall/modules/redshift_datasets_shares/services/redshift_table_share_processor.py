...
from dataall.modules.redshift_datasets_shares.aws.redshift_data import redshift_data_client, RedshiftData
...
class ProcessRedshiftShare(SharesProcessorInterface):
    def __init__(self, session, share_data, shareable_items, reapply=False):
        ...
        self.tables: List[RedshiftTable] = shareable_items  # TODO: use RedshiftTable
        ...
        self.target_admin_connection = RedshiftConnectionRepository.get_redshift_admin_connection(
            session=self.session,
            account_id=self.share_data.target_environment.AwsAccountId,
            region=self.share_data.target_environment.region,
            namespace=self.share_data.principal,  # TODO
        )
    ...
    def process_approved_shares(self) -> bool:
        ...
        if not self.tables:
            ...
            try:
                redshift_client_in_source = redshift_data_client(
                    account_id=self.share_data.source_environment.AwsAccountId,
                    region=self.share_data.source_environment.region,
                ...
                redshift_client_in_target = redshift_data_client(
                    account_id=self.share_data.target_environment.AwsAccountId,
                    region=self.share_data.target_environment.region,
