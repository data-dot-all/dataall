...
class RedshiftData:
    ...
    def describe_datashare(self, datashare: str):
        ...
        sql_statement = f'DESC DATASHARE {RedshiftData.identifier(datashare)};'
        # TODO: we need to transform the output
    ...
    def grant_usage_to_datashare(self, datashare: str, namespace: str):
        """
        Grant usage on datashare to cluster
        """
        log.info(f'Grant usage on {datashare=} to {namespace=}..')
        sql_statement = f"GRANT USAGE ON DATASHARE {RedshiftData.identifier(datashare)} TO NAMESPACE'{namespace}';"
        self._execute_statement(sql=sql_statement)
    ...
    def create_database_from_datashare(self, database: str, datashare: str, namespace: str):
        ...
        sql_statement = f"CREATE DATABASE {RedshiftData.identifier(database)} FROM DATASHARE {RedshiftData.identifier(datashare)} OF NAMESPACE'{namespace}';"
        self._execute_statement(sql=sql_statement)

    def create_external_schema(self, database: str, schema: str):
        log.info(f'Create external schema {schema=} in {database=}')
        sql_statement = f"CREATE EXTERNAL SCHEMA {RedshiftData.identifier(f'{database}_{schema}')} FROM DATABASE {RedshiftData.identifier(database)} SCHEMA {RedshiftData.identifier(schema)};"
        self._execute_statement(sql=sql_statement)
