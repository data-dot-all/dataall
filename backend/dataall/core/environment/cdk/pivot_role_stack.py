    ...
    def generate_policies(self) -> List[iam.ManagedPolicy]:
        ...
        for index, chunk in enumerate(statements_chunks):
            policies.append(
                iam.ManagedPolicy(
                    self.stack,
                    f'PivotRolePolicy-{self.environmentUri}-{index+1}',
                    managed_policy_name=f'{self.env_resource_prefix}-pivot-role-cdk-policy-{self.environmentUri}-{index+1}',
                    statements=chunk,
                )
            )
        return policies
...
class PivotRole(NestedStack):
    def __init__(self, scope: Construct, construct_id: str, config, **kwargs) -> None:
        ...
        self.role_name = f"{config['roleName']}-{self.environmentUri}"
    ...
    def create_pivot_role(self, principal_id: str, external_id: str) -> iam.Role:
        ...
        role = iam.Role(
            self,
            f'DataAllPivotRole-cdk-{self.environmentUri}',
            role_name=self.role_name,
