...
def get_stack_with_cfn_resources(targetUri: str, environmentUri: str):
    ...
    with context.db_engine.scoped_session() as session:
        ...
        stack: StackModel = StackRepository.find_stack_by_target_uri(session, target_uri=targetUri)
        if not stack:
            stack = StackModel(
                stack='environment',
                payload={},
...
def deploy_stack(targetUri):
    context = get_context()
    with context.db_engine.scoped_session() as session:
        stack: StackModel = StackRepository.get_stack_by_target_uri(session, target_uri=targetUri)
...
def delete_stack(target_uri, accountid, cdk_role_arn, region):
    context = get_context()
    with context.db_engine.scoped_session() as session:
        stack: StackModel = StackRepository.find_stack_by_target_uri(session, target_uri=target_uri)
        if not stack:
            return
        task = Task(
            targetUri=target_uri,
