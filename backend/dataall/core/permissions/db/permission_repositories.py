...
class Permission:
    @staticmethod
    def find_permission_by_name(session, permission_name: str, permission_type: str) -> models.Permission:
        if permission_name:
            permission = (
    ...
    @staticmethod
    def get_permission_by_name(session, permission_name: str, permission_type: str) -> models.Permission:
        if not permission_name:
            raise exceptions.RequiredParameter(param_name='permission_name')
        permission = Permission.find_permission_by_name(session, permission_name, permission_type)
        if not permission:
            raise exceptions.ObjectNotFound('Permission', permission_name)
        return permission

    @staticmethod
    def find_permission_by_uri(session, permission_uri: str, permission_type: str) -> models.Permission:
        if permission_uri:
            permission = (
                session.query(models.Permission)
                .filter(
    ...
    @staticmethod
    def get_permission_by_uri(session, permission_uri: str, permission_type: str) -> models.Permission:
        if not permission_uri:
            raise exceptions.RequiredParameter(param_name='permission_uri')
        permission = Permission.find_permission_by_uri(session, permission_uri, permission_type)
        if not permission:
            raise exceptions.ObjectNotFound('Permission', permission_uri)
        return permission

    @staticmethod
    def save_permission(session, name: str, description: str, permission_type: str) -> models.Permission:
        if not name:
    ...
    def init_permissions(session):
        ...
        count_resource_permissions = (
            session.query(models.Permission).filter(models.Permission.type == PermissionType.RESOURCE.name).count()
        )

        logger.debug(
            f'count_resource_permissions: {count_resource_permissions}, RESOURCES_ALL: {len(permissions.RESOURCES_ALL_WITH_DESC)}'
        )
        ...
        count_tenant_permissions = (
            session.query(models.Permission).filter(models.Permission.type == PermissionType.TENANT.name).count()
        )

        logger.debug(
            f'count_tenant_permissions: {count_tenant_permissions}, TENANT_ALL: {len(permissions.TENANT_ALL_WITH_DESC)}'
        )
