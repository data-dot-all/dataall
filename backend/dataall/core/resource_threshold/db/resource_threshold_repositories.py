...
class ResourceThresholdRepository(EnvironmentResource):
    ...
    def get_count_today(session, username, action_type):
        amount = session.query(ResourceTreshold.count).filter(
            and_(ResourceTreshold.username == username, ResourceTreshold.actionType == action_type, ResourceTreshold.date == func.current_date())).scalar()
    ...
    def add_entry(session, username, action_type):
        ...
        if user_entry:
            user_entry.update({ResourceTreshold.count: 1,ResourceTreshold.date : func.current_date()}, synchronize_session=False)
            session.commit()
        else:
    ...
    def increment_count(session, username, action_type):
        session.query(ResourceTreshold).filter(
            and_(ResourceTreshold.username == username, ResourceTreshold.actionType == action_type, ResourceTreshold.date == func.current_date()
    ...
    def invocation_handler(action_type):
        def decorator(func):
            def wrapper(session, username, *args, **kwargs):
                count = ResourceThresholdRepository.get_count_today(session=session, username=username, action_type=action_type)
                max_count = config.get_property('modules.worksheets.features.max_count_per_day', 10)
                if count < max_count:
                        response = func(session, *args, **kwargs)
                        if count == 0:
                            ResourceThresholdRepository.add_entry(session=session, username=username, action_type=action_type)
                        else:
                            ResourceThresholdRepository.increment_count(session=session, username=username, action_type=action_type)
                        return response
                else:
                    return {
            'error': None,
