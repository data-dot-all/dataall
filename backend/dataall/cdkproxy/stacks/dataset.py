...
class Dataset(Stack):
                ...
                share_items = (
                    ...
                    .filter(
                        and_(
                            ...
                            models.ShareObject.status == models.ShareObjectStatus.Approved.value,
                        )
                    )
                    .all()
                )
    ...
    def __init__(self, scope, id, target_uri: str = None, **kwargs):
        ...
        if env.dashboardsEnabled:
            quicksight_default_group = Quicksight.create_quicksight_default_group(dataset.AwsAccountId)
                    ...
                    statements=[
                        iam.PolicyStatement(
                            ...
                            principals=[
                                ...
                                iam.ArnPrincipal(f"arn:aws:iam::{env.AwsAccountId}:role/{pivot_role_name}"),
                            ],
                            actions=["kms:*"],
                        )
                    ],
        ...
        for location in shared_locations:
            ...
            dataset_bucket.encryption_key.grant_decrypt(grantee=iam.AccountPrincipal(location.AwsAccountId))
        ...
        dataset_admin_policy = iam.Policy(
            ...
            statements=[
                iam.PolicyStatement(actions=["s3:List*"], resources=["*"], effect=iam.Effect.ALLOW),
                iam.PolicyStatement(actions=["logs:*"], resources=["*"], effect=iam.Effect.ALLOW),
                iam.PolicyStatement(actions=["tag:*"], resources=["*"], effect=iam.Effect.ALLOW),
                iam.PolicyStatement(
                ...
                iam.PolicyStatement(actions=["kms:*"], effect=iam.Effect.ALLOW, resources=["*"]),
                iam.PolicyStatement(
        ...
        dataset_admin_role = iam.Role(
            ...
            assumed_by=iam.CompositePrincipal(
                ...
                iam.ArnPrincipal(f"arn:aws:iam::{dataset.AwsAccountId}:role/{pivot_role_name}"),
            ),
        )
        ...
        glue_db_dev = CustomResource(
            ...
            properties={
                "CatalogId": dataset.AwsAccountId,
                "DatabaseInput": {
                    "Description": "dataall development database {} ".format(dataset.GlueDatabaseName),
        ...
        glue_db_test = CustomResource(
            ...
            properties={
                "CatalogId": dataset.AwsAccountId,
                "DatabaseInput": {
                    "Description": "dataall test database {} ".format(dataset.GlueDatabaseName),
        ...
        job = glue.CfnJob(
            ...
            role=iam.ArnPrincipal(f"arn:aws:iam::{env.AwsAccountId}:role/{pivot_role_name}").arn,
