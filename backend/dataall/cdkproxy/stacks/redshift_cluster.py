...
class RedshiftStack(Stack):
    ...
    def get_target(self, target_uri):
        ...
        with engine.scoped_session() as session:
            cluster: models.RedshiftCluster = session.query(models.RedshiftCluster).get(
                target_uri
            )
    ...
    def get_env_group(self, cluster: models.RedshiftCluster) -> models.EnvironmentGroup:
        ...
        with engine.scoped_session() as session:
            env = Environment.get_environment_group(
                session, cluster.SamlGroupName, cluster.environmentUri
            )
        return env
    ...
    def __init__(self, scope, id: str, target_uri: str = None, **kwargs) -> None:
        ...
        if not cluster.imported:
            vpc = aws_ec2.Vpc.from_lookup(
                self, 'vpcRedshiftcluster', vpc_id=cluster.vpc
            )
            ...
            redshift_login = redshift.Login(
                master_username=master_secret.secret_value_from_json(
                    'username'
                ).to_string(),
                master_password=master_secret.secret_value_from_json('password'),
            )
            redshift_role = aws_iam.Role.from_role_arn(
                self, 'RedshiftRole', role_arn=env_group.environmentIAMRoleArn
            )
